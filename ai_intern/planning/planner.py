...
For this request, what tasks are needed?"""
    ...
    def create_plan(cls, request: RequestSchema) -> PlanSchema:
        ...
        logger.info(f"Planning Agent processing request: {request.request_id}")
        ...
        logger.debug("Fixing task plan_ids...")
        for task in plan.tasks:
            task.plan_id = plan.plan_id

        logger.info(f"Plan created: {plan.plan_id} with {len(plan.tasks)} tasks")
        logger.debug(f"Tokens consumed: {tokens_used}")
