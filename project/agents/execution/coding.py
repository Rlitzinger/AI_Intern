    ...
    def build_prompt(task: TaskSchema, context: dict = None) -> str:
        ...
        if context and context.get('previous_tasks'):
            ...
            for prev_task in context['previous_tasks']:
                ...
                # Include first 800 chars of result (enough for research data, not overwhelming)
                result_preview = prev_task['result'][:800] if prev_task['result'] else "No result"
...
Do NOT wrap in ```python``` or ``` blocks.""")
    ...
    def execute_task(cls, task: TaskSchema, context: dict = None) -> tuple[TaskSchema, int]:
        ...
        # DISABLED: This method is too aggressive and removes actual function code
        # TODO: Fix _strip_example_usage logic to only remove examples, not definitions
